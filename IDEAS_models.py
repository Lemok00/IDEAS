...
class DenoisingAutoencoder(nn.Module):
    def __init__(
        ...
        stem = [ConvLayer(3, channel, 1)]  # 32*256*256
        stem.append(ResBlock(channel, channel * 2, downsample=False, padding="reflect"))  # 64*256*256
        stem.append(ResBlock(channel * 2, channel * 4, downsample=False, padding="reflect"))  # 128*256*256
        stem.append(ResBlock(channel * 4, channel * 2, downsample=False, padding="reflect"))  # 64*256*256
        stem.append(ConvLayer(channel * 2, 3, 1, tanh=True))  # 3*256*256

        self.denoise = nn.Sequential(*stem)
