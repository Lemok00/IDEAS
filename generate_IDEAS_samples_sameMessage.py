...
if __name__ == "__main__":
    ...
    result_dir = f'results/generate_IDEAS_samples_sameMessage/{args.exp_name}/sigma={args.sigma}_delta={args.delta}'
    ...
    message = torch.randint(low=0, high=2, size=(1, ckpt_args.N * 16 * 16 * args.sigma))

    for i in  range(1, args.num + 1):
        with torch.no_grad():
            noise = message_to_tensor(message, sigma=args.sigma, delta=args.delta)
            noise = noise.reshape(shape=(1, ckpt_args.N, tensor_size, tensor_size)).to(device)
